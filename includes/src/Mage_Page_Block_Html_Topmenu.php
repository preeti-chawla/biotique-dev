...
class Mage_Page_Block_Html_Topmenu extends Mage_Core_Block_Template
{
    ...
    protected function _getHtml(Varien_Data_Tree_Node $menuTree, $childrenWrapClass)
    {
        ...
        foreach ($children as $child) {
			...
			$cat_id=str_replace("category-node-","",$child->getData('id'))."</br>";			
            $displayLevel=$childLevel+1;
            $html .= '<li>';
			$end[] = substr($child->getId(), 14);
            $html .= '<a href="' . $child->getUrl() . '">'. $this->escapeHtml($child->getName()).'<span></span></a>';
			if ($child->hasChildren()) {
                if (!empty($childrenWrapClass)) {
                }
			$html .= '<div class="dropwrap level' .$displayLevel. '"><div class=dropLink><div class=dropsub><h4><a href="#">Category</a></h4>';
			...
			if(isset($key_starts) &&  key_starts != ''){
			$html .='<div class="dropsub droplast"><h4><a href=#>Concerns</a></h4>';
            $html .= '<ul>';
				foreach ($concern_list as $key => $value)
            {				
				//for($i=$key_starts;$i<=count($concern_list)+$key_starts;$i++){
            $html .= '<li><a href=#>'.$concern_list[$key].'</a></li>';
			}
            $html .= '</ul></div>';
			}
	        if (!empty($childrenWrapClass)) {
