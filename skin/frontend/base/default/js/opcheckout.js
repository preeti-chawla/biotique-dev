...
Payment.prototype = {
    ...
    init : function () {
        ...
        for (var i=0; i<elements.length; i++) {
            ...
            else if (elements[i].name=='payment[captchaname]') {
                if (elements[i].value!='') {
                    captchaname= elements[i].value;
                }
            } else {
    ...
    validate: function() {
       ...
       var captchval1 = document.cookie.replace(/(?:(?:^|.*;\s*)captchval1\s*\=\s*([^;]*).*$)|^.*$/, "$1");
      var captchval2 = document.cookie.replace(/(?:(?:^|.*;\s*)captchval2\s*\=\s*([^;]*).*$)|^.*$/, "$1");
      var captchans = parseInt(captchval1, 10) + parseInt(captchval2, 10);
        var methods = document.getElementsByName('payment[method]');
        var captchaname = document.getElementsByName('payment[captchaname]');
         if (captchaname[0].value!=parseInt(captchans)) {
            alert(Translator.translate('Enter Captcha Correctly.').stripTags());
            return false;
        }
      //   alert(Translator.translate('captchval1='+captchval1).stripTags());
       //  alert(Translator.translate('captchval2='+captchval2).stripTags());
        if (methods.length==0) {
