...
class Biotique_Banner_Block_Adminhtml_Prodbanner_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{

	// gets executed after _prepareForm
	protected function _initFormValues()
	{
		if($prodbannerId = Mage::registry('current_slide')){
			$data = $prodbannerId->getData();
			$data['thumbnail_url'] = $data['image'];
			$this->getForm()->setValues($data);
		}

		// data preserving when failing
		if($data = Mage::getSingleton('adminhtml/session')->getData('prodbanner_form_data', true)){		// get data and clear form
			$this->getForm()->setValues($data);
		}
	}
