...
class Mage_Core_Model_Store extends Mage_Core_Model_Abstract
{
    ...
    public function getCurrentCurrencyCode()
    {
		...
		// End Get Client IP in $ip
		
		//For testing purpose overwrite $ip="76.109.14.196" for US country ip;
		//$ip="76.109.14.196";
		
		//Check session value for if current ip is already set in session
		//If current ip is not is session
		if($this->_getSession()->getMyIP()!=$ip){
			if(isset($ip)){
				
				//Set the current ip in session
				$this->_getSession()->setMyIP($ip);
			
				$xml = simplexml_load_file("http://www.geoplugin.net/xml.gp?ip=".$ip);
				//echo $xml->geoplugin_countryName ;
				
				//Based on GeoPlugin API set Website currency
				if($xml->geoplugin_countryName  == 'India') {       
					$this->_getSession()->setCurrencyCode('INR');
				} 
				else{
					$this->_getSession()->setCurrencyCode('USD');
				}
